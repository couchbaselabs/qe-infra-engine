...
def disconnect_slaves_parallel(slave_data, max_workers=None):
    if not max_workers:
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {executor.submit(disconnect_slave, slave): slave for slave in slave_data}
        for future in concurrent.futures.as_completed(futures):
            slave = futures[future]
            result = future.result()
            final_result[slave["ipaddr"]] = result

def disconnect_slave(slave):
    ipaddr = slave["ipaddr"]
    result = {}
    ...
    try:
        res_jenkins = jenkins_helper.disconnect_slave(slave_name, message="")
        logger.info(f"Slave with ipaddr {ipaddr} and name {slave_name} successfully disconnectted from jenkins")
    except Exception as e:
        result["result"] = False
        result["reason"] = f"Cannot disconnect slave {ipaddr}:{slave_name} from jenkins : {e}"
        logger.error(result["reason"])
        return result
    ...
    result["disconnect_slave_from_jenkins"] = res_jenkins
...
def main():
    ...
    results = disconnect_slaves_parallel(slave_data)
